...
type ColumnType = ColumnDef<FacilityInterface, unknown>;
...
export function FacilityListPage(props: FacilityListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getFacilities({
        relations: ['organization', 'asset.count', 'cleanup.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FacilityInterface>>(
    () => `/facilities?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FacilityInterface) => {
    if (hasAccess('facility', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/facilities/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('asset', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Asset',
          accessorKey: 'asset',
          cell: ({ row: { original: record } }: any) => record?._count?.asset,
        }
      : null,
    hasAccess('cleanup', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'cleanup',
          header: 'Cleanup',
          accessorKey: 'cleanup',
          cell: ({ row: { original: record } }: any) => record?._count?.cleanup,
        }
      : null,
