...
import { getAssets, deleteAssetById } from 'apiSdk/assets';
import { AssetInterface } from 'interfaces/asset';

type ColumnType = ColumnDef<AssetInterface, unknown>;
...
export function AssetListPage(props: AssetListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AssetInterface>>(
    () => `/assets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AssetInterface) => {
    if (hasAccess('asset', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/assets/view/${row.id}`);
    }
  };
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('asset', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/assets/edit/${record.id}`} passHref legacyBehavior>
              <Button
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="asset" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Asset
        </Text>
        {hasAccess('asset', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/assets/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
